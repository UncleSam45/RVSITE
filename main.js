(() => {
  ...
  function buildCheckoutPayload() {
    ...
    const giftItem = gift ? getItemById(gift.itemId) : null;
    ...
    const giftBlock = giftItem ? [
      '---',
      'CADEAU PROMOTIONNEL:',
      `${giftItem.gift_title || giftItem.title} — ${gift.portionLabel}`,
      'Prix promotionnel: GRATUIT',
      `Identifiant catalogue: ${giftItem.id}`,
      '---',
    ].join('\n') : '';
    const notes = [customer.notes || '', giftBlock, preferenceBlock].filter(Boolean).join('\n\n');
    return {
      items: state.cart.items
        .filter((line) => !isPromotionalItem(line) || getItemById(line.itemId)?.stripe_checkout_enabled !== false)
        .map((line) => {
          const item = getItemById(line.itemId);
          return { item_id: item?.checkout_item_id || line.itemId, portion: line.portion, qty: line.qty };
        }),
