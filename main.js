(() => {
    ...
    return `
      <header class="topbar container" id="topbar">
        ...
        <nav class="nav" aria-label="Navigation principale">
          ...
          <a class="nav-btn cart-nav" href="#commander" data-page="commander" aria-current="${state.page === 'commander'}">Panier <span class="cart-badge">${count}</span></a>
        </nav>
      </header>`;
  }
  ...
  function cartPanelHtml(includeButton = false) {
    ...
    return `<aside class="cart-panel card" aria-label="Votre panier"><h2>Votre panier</h2>
      ...
      ${includeButton ? `<a class="btn btn-primary" href="#commander" data-page="commander" style="width:100%;margin-top:12px">Voir le panier</a>` : ''}
    </aside>`;
  }
  ...
  function mobileCartBarHtml() {
    const totals = cartTotals();
    return `<div class="mobile-cart-bar ${totals.count ? '' : 'empty'}"><span>${totals.count} article${totals.count > 1 ? 's' : ''} • ${formatCurrency(totals.subtotal)}</span><a href="#commander" data-page="commander">Voir le panier</a></div>`;
  }
  ...
  async function init() {
    ...
    window.addEventListener('hashchange', () => { const page = window.location.hash.slice(1); if (PAGES.has(page) && page !== state.page) { state.page = page; render(); } });
