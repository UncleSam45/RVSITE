(() => {
  ...
  const state = {
    ...
    admin: { authenticated: false, token: '', owner: '', repo: 'RVSITE', branch: 'main', working: null, original: null, selectedFile: 'assets/data/content.json', message: '', saving: false, editorValid: true },
  };
  ...
  const ADMIN_FILES = [
    ['assets/data/content.json', 'Accueil & SEO'], ['assets/data/settings.json', 'Entreprise & règles'], ['assets/data/menus.json', 'Menu actif'], ['assets/data/items.json', 'Plats & prix'], ['assets/data/delivery.json', 'Livraison'], ['assets/data/promotions.json', 'Promotions'], ['assets/data/gallery.json', 'Galerie'],
  ];
  ...
  function adminHtml() {
    ...
    if (!admin.authenticated) {
      return `<section class="container section panel admin-panel"><div class="kicker">Console administrateur</div><h1 class="page-title">Accès sécurisé au contenu du site</h1><p class="lead">Entrez un jeton GitHub fine-grained autorisé à modifier ce dépôt. Le jeton reste dans ce navigateur et sert uniquement à appeler l’API REST de GitHub lorsque vous soumettez.</p><div class="form-grid"><div class="field"><label for="adminOwner">Propriétaire GitHub</label><input id="adminOwner" data-admin-auth="owner" value="${escapeHtml(admin.owner)}" placeholder="ex: votre-organisation"></div><div class="field"><label for="adminRepo">Dépôt</label><input id="adminRepo" data-admin-auth="repo" value="${escapeHtml(admin.repo)}"></div><div class="field"><label for="adminBranch">Branche</label><input id="adminBranch" data-admin-auth="branch" value="${escapeHtml(admin.branch)}"></div><div class="field"><label for="adminToken">Access key / token GitHub</label><input id="adminToken" class="admin-secret" data-admin-auth="token" value="${escapeHtml(admin.token)}" type="password" autocomplete="off" placeholder="github_pat_..."></div></div><div class="cta-row"><button class="btn btn-primary" data-admin-login>Ouvrir la console</button><button class="btn btn-secondary" data-page="home">Retour au site</button></div>${admin.message ? `<p class="notice">${escapeHtml(admin.message)}</p>` : ''}</section>`;
    }
    ensureAdminWorking();
    const path = admin.selectedFile;
    const value = JSON.stringify(admin.working[path], null, 2);
    const original = JSON.stringify(admin.original[path], null, 2);
    return `<div class="container admin-shell section"><aside class="panel admin-panel"><div class="kicker">RVSITE repo</div><h1 style="margin-top:6px">Console</h1><p>Modifiez les fichiers de données qui alimentent le site, vérifiez l’avant/après, puis soumettez vers GitHub.</p><div class="admin-tabs">${ADMIN_FILES.map(([file,label]) => `<button class="btn ${file === path ? 'btn-olive' : 'btn-secondary'} admin-tab" data-admin-file="${escapeHtml(file)}">${escapeHtml(label)}<small style="margin-left:auto">${escapeHtml(file)}</small></button>`).join('')}</div><div class="cta-row"><button class="btn btn-ghost" data-admin-reset>Réinitialiser</button><button class="btn btn-secondary" data-admin-logout>Verrouiller</button></div></aside><section class="panel admin-panel"><div class="section-head"><div><div class="kicker">Édition JSON foolproof</div><h2>${escapeHtml(path)}</h2><p>Les changements sont validés comme JSON avant d’être envoyés. Aucun fichier n’est modifié tant que vous ne cliquez pas sur Soumettre.</p></div></div><div class="field full"><label for="adminEditor">Contenu proposé</label><textarea id="adminEditor" class="admin-editor" data-admin-editor>${escapeHtml(value)}</textarea></div><div class="admin-diff"><div><strong>Avant</strong><pre>${escapeHtml(original)}</pre></div><div><strong>Après</strong><pre>${escapeHtml(value)}</pre></div></div><div class="cta-row"><button class="btn btn-secondary" data-admin-validate>Valider</button><button class="btn btn-primary" data-admin-submit ${(admin.saving || !admin.editorValid) ? 'disabled' : ''}>${admin.saving ? 'Soumission…' : 'Soumettre vers GitHub'}</button></div>${admin.message ? `<p class="notice ${admin.message.includes('succès') ? 'success-note' : ''}">${escapeHtml(admin.message)}</p>` : ''}</section></div>`;
  }
  ...
  function bindEvents(root) {
    ...
    root.querySelector('[data-admin-login]')?.addEventListener('click', () => {
      if (!state.admin.token || !state.admin.owner || !state.admin.repo || !state.admin.branch) { state.admin.message = 'Propriétaire, dépôt, branche et token sont requis.'; render(); return; }
      state.admin.authenticated = true; state.admin.message = ''; ensureAdminWorking(); render();
    });
    root.querySelector('[data-admin-logout]')?.addEventListener('click', () => { state.admin.authenticated = false; state.admin.token = ''; state.admin.message = 'Console verrouillée.'; render(); });
    ...
    root.querySelector('[data-admin-reset]')?.addEventListener('click', () => { state.admin.working = cloneJson(state.admin.original); state.admin.editorValid = true; state.admin.message = 'Changements locaux réinitialisés.'; render(); });
    root.querySelector('[data-admin-editor]')?.addEventListener('input', (event) => { try { state.admin.working[state.admin.selectedFile] = JSON.parse(event.target.value); state.admin.editorValid = true; state.admin.message = 'JSON valide. Cliquez Valider pour rafraîchir l’aperçu.'; } catch (error) { state.admin.editorValid = false; state.admin.message = `JSON invalide: ${error.message}`; } });
    root.querySelector('[data-admin-validate]')?.addEventListener('click', () => { state.admin.message = state.admin.editorValid ? 'JSON valide et prêt à soumettre.' : state.admin.message; render(); });
    root.querySelector('[data-admin-submit]')?.addEventListener('click', async () => {
      if (!state.admin.editorValid) { state.admin.message = 'Corrigez le JSON avant la soumission.'; render(); return; }
      state.admin.saving = true; state.admin.message = 'Soumission en cours vers GitHub…'; render();
      try { await githubPutJson(state.admin.selectedFile, state.admin.working[state.admin.selectedFile]); state.admin.original[state.admin.selectedFile] = cloneJson(state.admin.working[state.admin.selectedFile]); state.admin.message = 'Soumission GitHub réussie avec succès.'; }
      catch (error) { state.admin.message = `Erreur GitHub: ${error.message}`; }
