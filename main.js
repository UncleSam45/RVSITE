  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      *{box-sizing:border-box} html{scroll-behavior:smooth} body{margin:0;background:radial-gradient(circle at top left,#fffaf1 0,#F8F3EA 36%,#f3eadc 100%);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;-webkit-font-smoothing:antialiased} button,input,select,textarea{font:inherit} button{min-height:44px} a{color:inherit}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
      ...
      @media(max-width:660px){.stepper{grid-template-columns:1fr 1fr}.step-pill{text-align:left}.mobile-cart-bar.empty{display:none}.site{padding-left:8px;padding-right:8px}.topbar{border-radius:18px}.brand-mark{width:48px;height:48px;border-radius:16px;font-size:1.05rem}.hero{padding:20px;border-radius:24px}.hero-visual,.hero-visual img{min-height:270px}.grid-2,.grid-3,.grid-4,.footer-grid,.form-grid,.quote-list{grid-template-columns:1fr}.section-head{align-items:start;flex-direction:column}.menu-card{grid-template-columns:1fr}.menu-card img{height:210px}.portion-grid{grid-template-columns:1fr}.date-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.checkout-grid{gap:12px}.panel,.step{padding:16px}.brand-name{font-size:1rem}.brand-tagline{font-size:.76rem}}
    `;
    ...
    return `
      <section class="hero container">
        <div>
          ...
          <div class="trust-chips"><span class="chip">Fait maison</span><span class="chip">Livraison locale</span><span class="chip">Commande 48 h à l’avance</span><span class="chip">Portions Petit / Grand / Familial</span></div>
        </div>
      ...
      <section class="section container panel emotional-card">
        <div class="section-head"><div><div class="kicker">Entreprise locale</div><h2>Des repas faits maison, préparés avec soin pour les familles de la région.</h2><p>Livraison disponible à ${escapeHtml(zones)}. Chaque commande est planifiée avec attention pour simplifier les soupers de semaine.</p></div><button class="btn btn-primary" data-page="livraison">Voir les règles</button></div>
      </section>
      <section class="section container panel final-cta"><div class="kicker">Prêt à commander?</div><h2>Choisissez vos plats et planifiez votre livraison.</h2><p>Le menu de la semaine affiche les portions, les prix et les dates disponibles.</p><div class="cta-row" style="justify-content:center"><button class="btn btn-primary" data-page="menu">Voir le menu</button><button class="btn btn-secondary" data-page="commander">Planifier ma commande</button></div></section>`;
  }
