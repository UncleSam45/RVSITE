(() => {
  ...
  const LOCAL_IMAGE_LIBRARY = [
    { id: 'linguine-alfredo-poulet-grille', title: 'Linguine Alfredo au poulet grillé', category: 'Plat principal', image: 'assets/images/items/linguine-alfredo-poulet-grille/hero.webp', card: 'assets/images/items/linguine-alfredo-poulet-grille/card.webp', thumb: 'assets/images/items/linguine-alfredo-poulet-grille/thumb.webp' },
    { id: 'macaroni-3-fromages', title: 'Macaroni trois fromages', category: 'Favori familial', image: 'assets/images/items/macaroni-3-fromages/hero.webp', card: 'assets/images/items/macaroni-3-fromages/card.webp', thumb: 'assets/images/items/macaroni-3-fromages/thumb.webp' },
    { id: 'souvlaki-porc-patates-ail', title: 'Souvlaki de porc et patates à l’ail', category: 'Assiette maison', image: 'assets/images/items/souvlaki-porc-patates-ail/hero.webp', card: 'assets/images/items/souvlaki-porc-patates-ail/card.webp', thumb: 'assets/images/items/souvlaki-porc-patates-ail/thumb.webp' },
  ];
  ...
  function normalizeImageAsset(asset, index = 0) {
    if (!asset) return null;
    if (typeof asset === 'string') {
      return { id: `local-image-${index}`, title: 'Création maison', category: 'La cuisine de Rosalie', image: asset, card: asset, thumb: asset };
    }
    const image = asset.image || asset.hero || asset.card || asset.original || asset.thumb || '';
    if (!image) return null;
    return {
      id: asset.id || `local-image-${index}`,
      title: asset.title || asset.name || 'Création maison',
      category: asset.category || asset.badge || 'La cuisine de Rosalie',
      image,
      card: asset.card || image,
      thumb: asset.thumb || asset.card || image,
    };
  }

  function localImageAssets() {
    const configured = state.data?.content?.image_library;
    const source = Array.isArray(configured) && configured.length ? configured : LOCAL_IMAGE_LIBRARY;
    return source.map(normalizeImageAsset).filter(Boolean);
  }

  function curatedImageSlides() {
    const menuText = `${getCurrentMenu().title || ''} ${getCurrentMenu().description || ''} ${(getCurrentMenu().item_ids || []).join(' ')}`.toLowerCase();
    return localImageAssets().map((asset, index) => {
      const activeMatch = menuText.includes(asset.id.toLowerCase()) || menuText.includes(asset.title.toLowerCase());
      return {
        id: `local-gallery-${asset.id}`,
        title: asset.title,
        subtitle: activeMatch ? 'Mis en valeur avec le menu actuel.' : 'Inspiration maison pour vos commandes, familles et demandes traiteur.',
        image: asset.image,
        thumb: asset.thumb,
        status: activeMatch ? 'current' : 'custom',
        badge: activeMatch ? GALLERY_STATUS_LABELS.current : asset.category,
        cta_label: activeMatch ? 'Voir au menu' : 'Faire une demande',
        cta_page: activeMatch ? 'menu' : 'contact',
        enabled: true,
        sort: 500 + index,
        local: true,
      };
  ...
  function getGallerySlides() {
    ...
    const usedImages = new Set([...manualSlides, ...automaticSlides].map((slide) => slide.image));
    const localSlides = curatedImageSlides().filter((slide) => !usedImages.has(slide.image));
    return [...manualSlides, ...localSlides, ...automaticSlides];
  }
  ...
  function homeHtml() {
    ...
    const heroSlides = getGallerySlides();
    const heroItems = getCurrentMenuImageItems();
    const localFallbacks = localImageAssets();
    const activeHeroSlide = heroSlides.length ? heroSlides[state.carousel.index % heroSlides.length] : null;
    const activeHeroItem = heroItems.length ? heroItems[state.carousel.index % heroItems.length] : null;
    const activeLocalHero = localFallbacks.length ? localFallbacks[state.carousel.index % localFallbacks.length] : null;
    const heroImage = itemImagePath(activeHeroItem, 'hero') || activeHeroSlide?.image || state.data.content.home?.hero_image || activeLocalHero?.image || 'assets/images/items/linguine-alfredo-poulet-grille/hero.webp';
    const heroTitle = activeHeroItem?.title || activeHeroSlide?.title || activeLocalHero?.title || menu.title || 'Menu de la semaine';
    return `
      <section class="hero container">
        ...
        <div class="hero-visual">
          <img src="${escapeHtml(heroImage)}" alt="${escapeHtml(heroTitle)}" loading="eager" onerror="this.closest('.hero-visual').style.display='none'">
          <div class="hero-card"><strong>${escapeHtml(heroTitle)}</strong><span>${activeHeroItem ? 'Disponible cette semaine • ' : 'Inspiration maison • '}Petit / Grand / Familial • livraison à céduler avec le client</span></div>
        </div>
      </section>
  ...
  function imagePromotionHtml() {
    const assets = localImageAssets();
    if (assets.length < 2) return '';
    const start = state.carousel.index % assets.length;
    const ordered = [...assets.slice(start), ...assets.slice(0, start)].slice(0, 3);
    return `<section class="section container image-promo-grid" aria-label="Inspirations maison">
      <article class="image-promo-main"><img src="${escapeHtml(ordered[0].image)}" alt="${escapeHtml(ordered[0].title)}" loading="lazy"><div class="image-promo-label">${escapeHtml(ordered[0].title)} · parfait pour promouvoir les menus maison</div></article>
      <div class="image-promo-stack">${ordered.slice(1).map((asset) => `<article><img src="${escapeHtml(asset.card || asset.image)}" alt="${escapeHtml(asset.title)}" loading="lazy"><div class="image-promo-label">${escapeHtml(asset.category)} · ${escapeHtml(asset.title)}</div></article>`).join('')}</div>
    </section>`;
  }
