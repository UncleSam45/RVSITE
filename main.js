(() => {
  ...
  const APP_RELEASE = '20260904-fulfillment-v13';
  ...
  const CART_STORAGE_KEY = 'lacuisine_rosalie_cart_v3';
  const LEGACY_CART_STORAGE_KEY = 'lacuisine_rosalie_cart_v2';
  ...
  function loadCart() {
    try {
      ...
      if (typeof parsed.fulfillmentOptionId === 'string') state.cart.fulfillmentOptionId = parsed.fulfillmentOptionId;
      // v2 carts intentionally do not migrate either retired delivery window.
      state.cart.deliveryPolicyAccepted = parsed.deliveryPolicyAccepted === true;
      if (typeof parsed.deliveryPolicyVersion === 'string') state.cart.deliveryPolicyVersion = parsed.deliveryPolicyVersion;
  ...
  function clearCartAfterSuccessfulCheckout() {
    ...
    state.cart.fulfillmentOptionId = '';
    state.cart.deliveryPolicyAccepted = false;
    state.cart.deliveryPolicyVersion = '';
  ...
  function fulfillmentOptions() { return state.data?.delivery?.fulfillment_options || []; }
  function selectedFulfillment() { return fulfillmentOptions().find((option) => option.id === state.cart.fulfillmentOptionId && option.enabled !== false) || null; }
  function isPickup() { return selectedFulfillment()?.type === 'pickup'; }
  function deliveryPolicy() { return state.data?.delivery?.delivery_policy || {}; }
  ...
  function validateOrder() {
    ...
    if (!fulfillment) errors.push('Veuillez choisir un mode de livraison ou de ramassage.');
    if (!state.cart.deliveryPolicyAccepted) errors.push('Vous devez accepter la politique de livraison.');
    if (fulfillment?.requires_address) {
  ...
  function buildCheckoutPayload() {
    ...
    return {
      ...
      fulfillment_option_id: state.cart.fulfillmentOptionId,
      delivery_date: state.cart.deliveryDate,
      delivery_policy_accepted: state.cart.deliveryPolicyAccepted,
      delivery_policy_version: deliveryPolicy().version || '',
      cooler_available: pickup ? false : state.cart.coolerAvailable,
      delivery_instructions: state.cart.deliveryInstructions,
      customer: {
        name: customer.name, phone: customer.phone, email: customer.email,
        street_number: pickup ? '' : customer.streetNumber, street_name: pickup ? '' : customer.streetName,
        apartment: pickup ? '' : customer.apartment, city: pickup ? '' : customer.city,
        province: pickup ? '' : (customer.province || 'QC'), postal_code: pickup ? '' : (customer.postalCode || '').toUpperCase(),
  ...
  function itemPreviewHtml(item) {
    ...
    return `<article class="card menu-card home-menu-preview">${itemImageHtml(item)}<div class="menu-card-body"><span class="badge">${escapeHtml(item.category)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><div class="chip-row">${portions.map((p) => `<span class="chip">${p.label} ${formatCurrency(p.price)}</span>`).join('')}</div><button class="btn btn-ghost" data-page="menu">Voir dans le menu</button></div></article>`;
  }
  ...
  function dateSelectorHtml() {
    ...
    return `<p>Les dates de livraison disponibles respectent un délai minimal de préparation de 72h après votre commande.</p><p>${first ? `Prochaine livraison disponible: <strong>${formatDate(first)}</strong>.` : 'Aucune date de livraison disponible avec le délai de 72h.'}</p><div class="date-grid">${buttons.join('')}</div>${state.dateMessage ? `<p class="notice date-feedback">${escapeHtml(state.dateMessage)}</p>` : ''}<p class="line-meta">Les dates affichées respectent le délai minimal de préparation de 72h.</p><p class="line-meta">Jours de livraison: ${deliveryDayLabels()}.</p>`;
  }
  ...
  function customerFormHtml() {
    ...
    return `<div class="form-grid">
      ${input('name', 'Nom complet', 'autocomplete="name"')}${input('phone', 'Téléphone', 'autocomplete="tel"')}${input('email', 'Courriel', 'autocomplete="email" type="email" required')}<p class="line-meta">Le courriel est requis afin que Rosalie puisse vous contacter au besoin concernant votre commande.</p>${pickup ? '<p class="notice success-note full">Aucune adresse de livraison n’est requise pour le ramassage. Les instructions privées seront communiquées après le paiement.</p>' : `${input('streetNumber', 'Numéro civique')}${input('streetName', 'Rue', 'autocomplete="address-line1"')}${input('apartment', 'Appartement')}<div class="field"><label for="city">Ville</label><select id="city" data-customer="city"><option value="">Choisir une ville</option>${zones.map((zone) => `<option value="${escapeHtml(zone.city)}" ${c.city === zone.city ? 'selected' : ''}>${escapeHtml(zone.city)}</option>`).join('')}</select></div>${input('postalCode', 'Code postal', 'autocomplete="postal-code"')}`}
  ...
  function deliveryPolicyHtml(includeAcknowledgement = false) {
    const policy = deliveryPolicy();
    const body = Array.isArray(policy.body) ? policy.body : [policy.body].filter(Boolean);
    return `<aside class="delivery-policy" aria-labelledby="delivery-policy-title"><h3 id="delivery-policy-title">${escapeHtml(policy.title || 'Politique de livraison')}</h3>${body.map((text) => `<p>${escapeHtml(text)}</p>`).join('')}${includeAcknowledgement ? `<label class="policy-ack"><input type="checkbox" data-cart-checkbox="deliveryPolicyAccepted" ${state.cart.deliveryPolicyAccepted ? 'checked' : ''} required> <span>${escapeHtml(policy.acknowledgement_label || 'J’ai lu et compris les modalités de livraison ou de ramassage.')}</span></label>` : ''}</aside>`;
  }

  function deliveryInfoHtml() { return deliveryPolicyHtml(true); }

  function deliveryPreferencesHtml() {
    const options = fulfillmentOptions().filter((option) => option.enabled !== false);
    const pickup = isPickup();
    return `<div class="form-grid" style="margin-top:14px"><fieldset class="fulfillment-group full"><legend>Mode et horaire</legend>${options.map((option) => `<label class="fulfillment-card"><input type="radio" name="fulfillment-option" data-fulfillment-option value="${escapeHtml(option.id)}" ${state.cart.fulfillmentOptionId === option.id ? 'checked' : ''}><span><strong>${escapeHtml(option.label)}</strong></span></label>`).join('')}</fieldset>
      ${pickup ? '' : `<label class="field full policy-ack"><input type="checkbox" data-cart-checkbox="coolerAvailable" ${state.cart.coolerAvailable ? 'checked' : ''}> <span>Je peux laisser ma glacière à l’extérieur pour préserver la fraîcheur des plats si je suis absent(e).</span></label>`}
      <div class="field full"><label for="deliveryInstructions">Instructions pour la livraison ou le ramassage</label><textarea id="deliveryInstructions" data-cart-field="deliveryInstructions">${escapeHtml(state.cart.deliveryInstructions || '')}</textarea></div></div>`;
  }

  function checkoutDeliverySummaryHtml() {
    const option = selectedFulfillment();
    return `<div class="cart-lines"><p class="line-meta"><strong>Date:</strong> ${state.cart.deliveryDate ? formatDate(state.cart.deliveryDate) : 'Aucune date disponible'}</p><p class="line-meta"><strong>Mode et horaire:</strong> ${escapeHtml(option?.label || 'À choisir')}</p>${option?.type === 'delivery' ? `<p class="line-meta"><strong>Glacière disponible:</strong> ${state.cart.coolerAvailable ? 'Oui' : 'Non'}</p>` : ''}${state.cart.deliveryInstructions ? `<p class="line-meta"><strong>Instructions:</strong> ${escapeHtml(state.cart.deliveryInstructions)}</p>` : ''}</div>`;
  }
  ...
  function bindEvents(root) {
    ...
    root.querySelectorAll('[data-cart-checkbox]').forEach((field) => field.addEventListener('change', () => { state.cart[field.dataset.cartCheckbox] = field.checked; if (field.dataset.cartCheckbox === 'deliveryPolicyAccepted') state.cart.deliveryPolicyVersion = field.checked ? (deliveryPolicy().version || '') : ''; saveCart(); render(); }));
    root.querySelectorAll('[data-fulfillment-option]').forEach((field) => field.addEventListener('change', () => { state.cart.fulfillmentOptionId = field.value; if (isPickup()) state.cart.coolerAvailable = false; saveCart(); render(); }));
  ...
  async function init() {
    ...
    if (!fulfillmentOptions().some((option) => option.id === state.cart.fulfillmentOptionId && option.enabled !== false)) state.cart.fulfillmentOptionId = '';
    if (state.cart.deliveryPolicyVersion !== deliveryPolicy().version) { state.cart.deliveryPolicyAccepted = false; state.cart.deliveryPolicyVersion = ''; }
    if (isPickup()) state.cart.coolerAvailable = false;
