(() => {
  ...
  function menuBannerHtml() {
    const bannerPaths = ['banner.png', 'assets/banner.png', 'assets/images/banner.png', 'assets/images/hero/banner.png'];
    return `<section class="menu-banner" aria-label="Real Food. Real Convenience."><img src="${bannerPaths[0]}" data-banner-paths="${escapeHtml(bannerPaths.join('|'))}" data-banner-index="0" alt="Real Food. Real Convenience. Homemade meals, ready when you are." loading="eager" onerror="const paths=this.dataset.bannerPaths.split('|');const next=Number(this.dataset.bannerIndex||0)+1;if(next<paths.length){this.dataset.bannerIndex=String(next);this.src=paths[next];}else{this.closest('.menu-banner').style.display='none';}"></section>`;
  }
