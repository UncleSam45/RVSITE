...
const smallOrder = validateOrder({ ...base, items: [line('paid', 'familial', 4), line('small', 'petit')] }, site);
...
const familyOrder = validateOrder({ ...base, items: [line('paid', 'familial', 6), line('family', 'familial')] }, site);
assert.equal(familyOrder.promotionTier, 'familial');
assert.throws(() => validateOrder({ ...base, items: [line('paid', 'familial', 6), line('small', 'petit')] }, site), /palier/);
assert.throws(() => validateOrder({ ...base, items: [line('paid', 'familial', 4)] }, site), /Choisissez/);
