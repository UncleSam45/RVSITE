...
def read_upload_bytes(source: Any) -> bytes | None:
    if source is None:
        return None
    ...
    if file_obj is not None and file_obj is not source:
        data = read_upload_bytes(file_obj)
        if data is not None:
            return data

    for attribute in ("content", "_data"):
        data = read_upload_bytes(getattr(source, attribute, None))
        if data is not None:
            return data
...
def upload_event_bytes(event: Any) -> bytes:
    for source in (event, upload_event_file(event)):
        data = read_upload_bytes(source)
        if data:
            return data
    ...
    with ui.column().classes("shell w-full").style("gap:16px"):
        ...
        with ui.tab_panels(tabs, value=business_tab).classes("w-full"):
            ...
            with ui.tab_panel(items_tab):
                ...
                def upload_item_image(event: Any) -> None:
                    ...
                    temp_path.write_bytes(upload_event_bytes(event))
                    try:
            ...
            with ui.tab_panel(gallery_tab):
                ...
                def upload_gallery_image(event: Any) -> None:
