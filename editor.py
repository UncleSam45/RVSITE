...
def main() -> int:
    try:
        ...
        log_file = BASE_DIR / "editor_startup_error.log"
        error_text = (
            ...
            "3) If this machine has no desktop UI, run on a machine with GUI support.\n"
            f"4) Detailed log written to: {log_file}\n"
        )
        ...
        try:
            log_file.write_text(f"{error_text}\n{traceback_text}\n", encoding="utf-8")
        except Exception:
            pass
        if sys.stdin.isatty():
            try:
